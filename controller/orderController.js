...
const { orderDeliveredBody } = require('../lib/email-sender/templates/order-delivered'); // Create this template
...
const getAllOrders = async (req, res) => {
  ...
  try {
    ...
    const orders = await Order.find(queryObject)
      .select(
        "_id invoiceDetails paymentMethod subTotal total user_info discount shippingCost status createdAt updatedAt"
      )
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limits);
...
//         success: false,
//         message: `Error occurred when uploading the file: ${err.message}`,
//       });
//     }
...
//         const userEmail = order.user_info.email;
//         // Email subject and body
//         const option = {
//           email: userEmail,
//           orderId: id, // Add any other necessary details
//         };
//         const body = {
//           from: process.env.EMAIL_USER,
//           to: userEmail,
//           subject: 'Your Order has been Delivered',
//           html: orderDeliveredBody(option),
//           attachments: [
//             {
...
//           ]
//         };
//         const message = "Order status updated to delivered and email sent.";
//         // Update order status in database
//         order.status = status;
//         await order.save();
//         //mail send
//         console.log(filePath);
//         await sendEmail(body, res, message,filePath); // Use your existing sendEmail function
...
//       res.status(500).send({
//         success: false,
//         message: `Error occurred when updating the order status: ${err.message}`,
//       });
//     }
//   });
// };
...
const orderstatuschange = async (req, res) => {
  ...
  uploadSingle(req, res, async (err) => {
    ...
    const { id, status } = req.body;
    try {
      const order = await Order.findOne({ _id: id }, { "user_info.email": 1 }).exec();
      ...
      if (status === 'Delivered') {
        ...
        // Send email
        const emailSent = await sendEmail(body, filePath);
...
const search = async (req, res) => {
  ...
  if (stuffRole) {
    query["user_info.stuffRole"] = stuffRole;
      ...
      const filteredOrders = await Order.find(query).skip((page - 1) * limit).exec();
