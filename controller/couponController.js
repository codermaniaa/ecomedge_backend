...
const getAllCoupons = async (req, res) => {
  ...
  try {
    ...
    const coupons = await Coupon.find(queryObject).sort({ _id: -1 }).skip((page - 1) * limit).limit(limit).exec();
    const allcoupons = await Coupon.find(queryObject).exec();

    var count=allcoupons.length;
